...
class MutualFundDetail(MutualFundRead):
    nav_history: list[NAVHistoryRead] = Field(default_factory=list)
...
class TrendingFundsResponse(BaseModel):
    """
    Real trailing-CAGR ranking. `items` only contains funds with enough
    NAV history to actually compute the ranking metric — funds lacking
    sufficient history are omitted rather than assigned a fabricated
    value, per the platform's data-availability policy.
    """

    ranking_metric: str = "cagr_3y"
    years: float = 3.0
    items: list[MutualFundRead]
